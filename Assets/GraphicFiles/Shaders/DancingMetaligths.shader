Shader "DancingMetalights"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			static float beat = 0.0;
			float mb(float2 p1, float2 p0)
			{ 
            ...
            fixed4 frag (pixel i) : SV_Target
			{
				...
				float2 mbr,mbg,mbb;
				float2 p = coordinate;
				float2 o = float2(pow(p.x,2.),pow(p.y,2.));
				float valueCol = pow(2.*abs(o.x+o.y)+abs(o.x-o.y), 5.0);
				float3 col = float3(valueCol, valueCol, valueCol);
				col = max(col,1.);
				float t= TIME +beat*2.;
				...
				mbr = mbg = mbb = float2(0.0, 0.0);
				mbr += float2(0.10*s4+0.40*c3,0.40*s2 +0.20*c3);
				mbg += float2(0.15*s3+0.30*c2,0.10*-s4+0.30*c3);
				mbb += float2(0.10*s3+0.50*c3,0.10*-s4+0.50*c2);
